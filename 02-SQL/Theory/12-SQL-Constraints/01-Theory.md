...
Suppose you have a Students table — `student_id`, `name`, `age`

Now if someone:

- leaves `student_id` empty
- enters the same `student_id` twice
- enters a negative age
- inserts invalid values

then it will create data problems in the database.

To prevent these issues, we use constraints.
